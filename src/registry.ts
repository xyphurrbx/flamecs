...
export const registry = ecs.world();
...
function hookListeners(id: Entity): void {
	...
	registry.set(id, ecs.OnAdd, (entity: ecs.Entity, _id: ecs.Id, data: unknown) => {
		addedSignal.fire(entity);
	});
	registry.set(id, ecs.OnRemove, (entity: ecs.Entity, _id: ecs.Id, _deleted?: true) => {
		removedSignal.fire(entity);
	});
	registry.set(id, ecs.OnChange, (entity: ecs.Entity, _id: ecs.Id, data: unknown) => {
 ...
 * @metadata macro
 */
export function reserve<T>(runtimeId: Entity, key?: ComponentKey<T>): void {
	assert(key);
...
export function component<T>(key?: ComponentKey<T>): Entity<Unwrap<T>> {
	...
	if (id === undefined) {
		id = registry.component<Unwrap<T>>();
...
export function reset(): void {
	const temporaryRegistry = ecs.world();
	for (const [key, value] of pairs(temporaryRegistry as unknown as Record<string, unknown>)) {
...
reserve<Wildcard>(ecs.Wildcard);
reserve<ChildOf>(ecs.ChildOf);
